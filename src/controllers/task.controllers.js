...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const verificationLink = `http://localhost:${process.env.PORT}/api/verify-email?token=${emailToken}`;
  ...
  const mailOptions = {
    ...
    html: `
            <p>Hello,</p>
            <p>Please click on the link below to verify email address:</p>
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!user) {
   ...
   if (user.isVerified == false) {
    ...
    throw new apiError(400, "Email not verified; Account removed from database");
  }
...
const createTask = asyncHandler(async (req, res) => {
  ...
  const task = await Task.create({
    taskName,
    taskDescription:
      taskDescription.trim() === "" ? "No description" : taskDescription,
...
const getAllTasksByUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new apiResponse(200, allTasks, "All Tasks retrived successfully"));
});
...
const updateTask = asyncHandler(async (req, res) => {
  ...
  if ([_id, taskName].some((field) => field?.trim() === "")) {
